...
import { collection, doc, query, onSnapshot, addDoc, updateDoc } from "firebase/firestore";
...
const Moves = () => {
  ...
  useEffect(() => {
    ...
    const fetchCurrentTime = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/current-time`);
        const data = await response.json();
  ...
  const handleSelectMove = async (move) => {
    ...
    try {
      const moveDocRef = doc(firestore, "moves", move.id); // Ensure the correct document reference
      await updateDoc(moveDocRef, {
        status: "picked up",
        picked_up_at: new Date().toISOString(), // Add the picked-up timestamp
      });
      console.log(`Move ${move.id} successfully updated with picked_up_at timestamp.`);
    } catch (err) {
  ...
  const handleCancelMoveCompletion = async () => {
    if (selectedMove) {
      try {
        ...
        await updateDoc(moveDocRef, { status: "open" }); // Reset status to open
  ...
  const handleSubmitTo = async () => {
    ...
    try {
      const moveDocRef = doc(firestore, "moves", selectedMove.id); // Ensure the correct document reference
      await updateDoc(moveDocRef, {
        to_location: toOptions.to_location,
        to_door: toOptions.to_door,
        status: "completed",
        completed_at: new Date().toISOString(), // Add the completed timestamp
      });
      setSuccessMessage("Move updated successfully!");
      console.log(`Move ${selectedMove.id} successfully updated with completed_at timestamp.`);
      setSelectedMove(null);
      setToOptions({ to_location: "", to_door: "" });
    } catch (err) {
